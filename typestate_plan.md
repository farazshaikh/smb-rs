...
- **Phase 0 — scaffolding.** New `smb-server/src/io/` module with the marker
  traits, `IoState`, `Origin`, `SmbResponse`, `ReplyHeader`, `SealIntent`. No
  wiring yet. Pure additions; nothing calls them.
- **Phase 1 — `Decode` + `SmbRequest`.** Implement `Decode` for the commands
  whose request structs already exist (`CreateReq`, `LockReq`, `ReadReq`, …) and
  add a `SmbRequest::parse`. Add a debug-only assertion in `process_single` that
  the new parser agrees with the current inline parse (equivalence test), then
  remove it.
- **Phase 2 — `IoContext` for leaf commands.** Route `ECHO`, `FLUSH`,
  `TREE_DISCONNECT`, `LOGOFF` through `accept → serve → into_response`. These have
  no async and no state, so they exercise the happy path end-to-end.
- **Phase 3 — migrate command clusters.** Move handlers into `Command` impls one
  cluster at a time (create/close, read/write, query/set-info, query-dir,
  ioctl). The old `match` arm becomes `CreateCmd::serve(ctx, r).await`. Compound
  handling wraps a `Vec<IoContext<Accepted>>`.
- **Phase 4 — `Pending`.** Model STATUS_PENDING for `LOCK` waits and
  `CHANGE_NOTIFY` with `Outcome::Interim`/`IoContext<Pending>`; the async task
  owns the parked context and calls `complete`. Replaces the `async_cancels` /
  `async_msgids` bookkeeping with an owned context per pending op.
- **Phase 5 — `Unsolicited`.** Re-express `send_lease_break` /
  `send_oplock_break` as `IoContext<_, Unsolicited>::from_event(...).into_response()`;
  the SessionId=0 and unsigned-break rules live in one `Encode`/header path.
- **Phase 6 — retire the old dispatch.** Once every command is a `Command`,
  delete the legacy `match` in `process_single`; `process_frame` becomes:
  frame → `SmbRequest::parse` → `accept` → `serve` → `encode`.

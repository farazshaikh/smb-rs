...
use smb_server_proto::buf::Writer;
...
impl QueryMeta {
    ...
    pub fn from_vfs(m: &smb_server_vfs::FileMeta) -> QueryMeta {
        QueryMeta {
...
pub fn encode_fs_info(class: u8) -> Option<Vec<u8>> {
    ...
    match class {
        fs_class::VOLUME => {
            d.push_u64(smb_server_proto::types::FileTime::now().0); // CreationTime
...
pub fn decode_set_file_op(class: u8, buf: &[u8]) -> Result<Option<smb_server_vfs::SetOp>, DecodeSetInfoError> {
    ...
    match class {
        file_class::BASIC => {
            ...
            let ft = |v: u64| (v != 0 && v != u64::MAX).then_some(smb_server_proto::types::FileTime(v));
            Ok(Some(smb_server_vfs::SetOp::Basic {
                access: ft(r64(8)),
                write: ft(r64(16)),
            }))
        }
        file_class::END_OF_FILE => Ok(Some(smb_server_vfs::SetOp::EndOfFile(r64(0)))),
        file_class::ALLOCATION => Ok(Some(smb_server_vfs::SetOp::Allocation(r64(0)))),
        file_class::DISPOSITION => Ok(Some(smb_server_vfs::SetOp::Disposition {
        ...
        file_class::RENAME => {
            ...
            Ok(Some(smb_server_vfs::SetOp::Rename { replace_if_exists: replace, name }))
        }
